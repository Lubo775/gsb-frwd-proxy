...
data "aws_security_group" "dynatrace_clb_sg" {
    filter {
        name   = "vpc-id"
        values = ["${data.aws_vpc.default.id}"]
    }
...
resource "aws_security_group" "AutoscalingInstancesSG" {
    ...
    description = "GSBProxy-Dynatrace ec2 SG"

    # TCP access on port 9999 for ActiveGate traffic
    ingress {
        from_port       = 9999
        to_port         = 9999
        protocol        = "tcp"
        security_groups = [ "${data.aws_security_group.dynatrace_clb_sg.id}" ]
    }
...
data "template_file" "must_run_scripts" {
    ...
    vars     = {
        ...
        active_gate_clb      = var.active_gate_clb
        apmaas_vpc_endpoint  = var.apmaas_vpc_endpoint
...
data "aws_ami" "activegate_latest" {
  most_recent = true
  owners = ["335913599016"]

  filter {
    name   = "name"
    values = ["gsbproxy-activegate-ami-*"]
  }
...
resource "aws_launch_template" "LaunchTemplate" {
  name_prefix   = "lt-gsb-dynatrace"
  image_id      = "${data.aws_ami.activegate_latest.id}"
...
resource "aws_cloudwatch_metric_alarm" "UnhealthyHostCount" {
  alarm_name                = "GSBProxyUnhealthyHost-Dynatrace"
