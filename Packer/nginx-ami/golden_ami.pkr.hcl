...
source "amazon-ebs" "golden_ami" {
  ami_name  = "gsbproxy-nginx-ami-${local.timestamp}"
...
build {
    ...
    provisioner "shell" {
      ...
      scripts = [
        ...
        "../scriptsForPacker/OneAgent.sh",
        "../scriptsForPacker/Nginx.sh",
    ...
    provisioner "shell" {
      inline = ["sudo mkdir /Licenses", "sudo mv /tmp/LICENSE.txt /Licenses/"]
      execute_command = "{{.Vars}} bash '{{.Path}}'"
    }
}
